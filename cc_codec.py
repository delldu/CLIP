...
#
# Create C/C++ codec
# python cc_codec.py | tee project/clip_codec.cpp
...
def create_byte_encoder_code():
    '''
        std::unordered_map<uint32_t, std::string> codec_byte_encoder {
            {33, "!"},
            {174, "®"},
        };
    '''
    count = 0
    lines = []
    print("std::unordered_map<uint32_t, std::string> codec_byte_encoder {")
    for k, v in codec.byte_encoder.items():
        lines.append(f"{{{k:3}, \"{x(v)}\"}},")
        if (count + 1) % COLS == 0:
            print("    ", " ".join(lines))
            lines = []
        count = count + 1
...
def create_word_encoder_code():
    '''
        std::unordered_map<std::string, uint32_t> codec_word_encoder {
            {"a</w>", 320},
            {"hello</w>", 3306},
            {"diagram</w>", 22697},
        };
    ...
    count = 0
    lines = []    
    print("std::unordered_map<std::string, uint32_t> codec_word_encoder {")
...
def create_bpe_ranks_code():
    '''
        std::unordered_map<std::string, uint32_t> codec_bpe_ranks {
            {('i', 'n'), 0},
        };
    '''
    count = 0
    lines = []    
    print("std::unordered_map<std::string, uint32_t> codec_bpe_ranks {")
...
def create_byte_decoder_code():
    '''
        std::unordered_map<std::string, uint32_t> codec_byte_decoder {
            {"!", 33},
            {"®", 174},
        };
    '''
    count = 0
    lines = []    
    print("std::unordered_map<std::string, uint32_t> codec_byte_decoder {")
    for k, v in codec.byte_decoder.items():
        lines.append(f"{{\"{x(k)}\", {v:3}}},")
        if (count + 1) % COLS == 0:
            print("    ", " ".join(lines))
            lines = []
        count = count + 1
...
def create_word_decoder_code():
    '''
        std::unordered_map<uint32_t, std::string> codec_word_decoder {
            {320, "a</w>"},
            {3306, "hello</w>"},
            {22697, "diagram</w>"},
        };
    ...
    count = 0
    lines = []
    print("std::unordered_map<uint32_t, std::string> codec_word_decoder {")
